<?php $pageTitle='Monthly Report'; $cp='/reports/monthly'; ob_start(); ?>
<div class="flex-center gap-2 mb-2">
  <form method="GET" class="flex-center gap-1">
    <label style="text-transform:none;letter-spacing:0;font-weight:400;font-size:.9rem">Month:</label>
    <input type="month" name="month" value="<?= e($month) ?>" style="width:auto">
...
<p class="text-muted text-sm mb-2"><?= date('F Y', strtotime($start)) ?></p>

<div class="stats-grid">
  <div class="stat-card revenue"><div class="stat-label"><i class="fa-solid fa-coins" aria-hidden="true"></i> Monthly Revenue</div><div class="stat-value"><?= money($sum['revenue']??0) ?></div></div>
  ...
  <div class="card">
    ...
    <div class="table-wrap"><table>
      ...
      <tbody>
        ...
        <tr><td class="font-bold text-sm"><?= e($r['staff_name']) ?></td><td><?= $r['num_sales'] ?></td><td class="text-accent font-bold"><?= money($r['revenue']) ?></td></tr>
        <?php endforeach; ?>
        <?php if(empty($byStaff)): ?><tr><td colspan="3" class="text-muted text-sm" style="padding:1rem">No data</td></tr><?php endif; ?>
      </tbody>
    </table></div>
  </div>
</div>
...
<div class="card">
  <div class="card-header"><h3><i class="fa-solid fa-trophy" aria-hidden="true"></i> Top Selling Products — <?= date('F Y', strtotime($start)) ?></h3></div>
  <div class="table-wrap"><table>
    <thead><tr><th>Product</th><th>Category</th><th>Gender</th><th>Size</th><th>Units Sold</th><th>Revenue</th></tr></thead>
    <tbody>
      <?php foreach($topSellers as $p): ?>
      <tr>
        <td><div class="font-bold text-sm"><?= e($p['name']) ?></div><div class="text-muted" style="font-size:.72rem"><?= e($p['design']??'') ?></div></td>
        <td class="text-sm"><?= e($p['category_name']) ?></td>
        ...
        <td class="text-accent font-bold"><?= money($p['revenue']) ?></td>
      </tr>
      <?php endforeach; ?>
      <?php if(empty($topSellers)): ?><tr><td colspan="6" style="text-align:center;padding:1.5rem;color:var(--muted)">No sales this month.</td></tr><?php endif; ?>
    </tbody>
  </table></div>
</div>
